...
class ScribbleHubScraper(BaseScraper):
    ...
    def fetch_chapter_content(self, chapter_url):
        ...
        title = (
            self.clean_chapter_title(title_tag.get_text(strip=True))
            if title_tag
        ...
        if published_tag and published_tag.has_attr("datetime"):
            dt = datetime.fromisoformat(
                published_tag["datetime"].replace("Z", "+00:00")
            )
        ...
        if content_div:
            texts = [
                ...
                if t.strip() not in self.ANTISCRAPES
            ]
            content = "\n".join(texts)
        else:
