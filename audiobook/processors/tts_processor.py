...
class TTSProcessor:
    ...
    def __init__(self, file_name, config, output_dir, tmp_dir, max_chunk_size=250):
        ...
        self.output_path_mp3 = os.path.join(
            self.output_dir, f"{self.base_output_file}.mp3"
        )
    ...
    def _load_speakers(self):
        ...
        return [
            os.path.splitext(f)[0] for f in os.listdir("speakers") if f.endswith(".wav")
        ]
    ...
    def convert_text_to_speech(self):
        ...
        for idx, part in enumerate(parts):
            match = re.search(
                r"<<SPEAKER=([^>]+)>>(.+?)<</SPEAKER>>", part, flags=re.DOTALL
            )
            if match:
                name = (
                    self.narrator
                ...
                if not os.path.exists(out_wav_path):
                    ...
                    if is_system:
                        ...
                        if self.system.get("speed", 1.0) != 1.0:
                            out_wav_path = change_playback_speed(
                                out_wav_path, self.system["speed"]
                            )
